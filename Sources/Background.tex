...
We find the best weights by minimizing the loss function $\mathcal{L}$ that
determines the quality of the model. There are many different loss functions.
However, we only describe the most common ones for classification and regression
tasks.
...
$$
    \text{MSE} = \frac{1}{n}\sum\limits^{n}_{i=1}(y_i - a_i)^2
$$

\bld{Cross-entropy loss (CE)} is widely used for classification. For simplicity,
consider a binary classification where the values are either 1 or 0. Consider
again the dataset $\mathcal{D}$ and predictions $y_i$. The CE is then defined as:
$$
    \text{CE} = -\frac{1}{n}\sum\limits_{n=1}^{n} a_i \log(y_i) + (1-a_i)
    \log(1-y_i)
$$
motivation
...
\footnotetext{The terms validation and test set are sometimes used
    interchangeably. However, the true meaning is not important for the
    illustration. Both are unobserved data.}
