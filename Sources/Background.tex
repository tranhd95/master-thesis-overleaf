...
their \bld{class} (also called category or label). For illustration see
\myref{Figure}{fig:od}. Generally, the task can be broken down into three steps:
...
We describe this process in detail in \myref{Section}{sec:training}.
...
In the following subsections we describe the layers of \bld{convolutional neural
    networks (CNNs} that are a special case of a feedforward networks. They thrive at
