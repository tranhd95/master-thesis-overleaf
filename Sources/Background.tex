...
the minibatch, $\nabla_\mathcal{L_\mathcal{D}(\theta)}$ be the gradient of
$\mathcal{L}_\mathcal{D}(\theta)$, and $\theta_t$ denotes the weights in the
step $t$ (similarly for other variables). The algorithm can be writen as:
$$
    \begin{array}{rl}
        \Delta\theta_{t} & = - \epsilon_{t} \cdot \nabla_{\mathcal{L}_\mathcal{D}(\theta_{t})} \\
...
\bld{AdaDelta} \cite{adadelta}, \bld{RMSProp}
\footnote{
    It was not published. However, it was proposed in a course by Geoff Hinton
    \url{
...
\footnotetext[3]{The simplest explanation is most likely the right one.}
